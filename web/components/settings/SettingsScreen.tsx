  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#131315] border border-[#1F1F23] rounded-xl p-6 space-y-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              >
                <option value="nvidia_a100">NVIDIA Tensor A100 GPU (80GB VRAM)</option>
                <option value="nvidia_t4">NVIDIA Tesla T4 GPU (16GB VRAM)</option>
                <option value="tpu_v4">Google Cloud TPU v4 (Supercomputer class)</option>
                <option value="cpu_only">CPU Only node allocation</option>
              </select>
            </div>
        ...
        <div className="bg-[#131315] border border-[#1F1F23] rounded-xl p-6 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <button onClick={() => setEnableAutoSave(!enableAutoSave)} className="cursor-pointer">
                {enableAutoSave ? (
                  <ToggleRight className="w-8 h-8 text-[#32D583]" />
                ) : (
                  <ToggleLeft className="w-8 h-8 text-[#c0c7d5]/60" />
                )}
              </button>
            </div>
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <button onClick={() => setEnableSlack(!enableSlack)} className="cursor-pointer">
                {enableSlack ? (
                  <ToggleRight className="w-8 h-8 text-[#32D583]" />
                ) : (
                  <ToggleLeft className="w-8 h-8 text-[#c0c7d5]/60" />
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <button onClick={() => setEnableTelemetry(!enableTelemetry)} className="cursor-pointer">
                {enableTelemetry ? (
                  <ToggleRight className="w-8 h-8 text-[#32D583]" />
                ) : (
                  <ToggleLeft className="w-8 h-8 text-[#c0c7d5]/60" />
      ...
      <div className="flex justify-end space-x-4 pt-4 border-t border-[#1F1F23]/60">
        ...
        <button
          onClick={handleSave}
          className="bg-[#3192fc] hover:brightness-110 text-white text-xs font-bold px-5 py-2.5 rounded-lg flex items-center space-x-2 shadow-lg transition-all cursor-pointer"
        >
