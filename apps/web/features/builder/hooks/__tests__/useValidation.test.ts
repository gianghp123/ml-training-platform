...
const mockedValidateGraph = validateGraph as jest.MockedFunction<typeof validateGraph>;
...
describe('useValidation', () => {
  ...
  it('returns errors for disconnected input ports', () => {
    ...
    expect(result.current.getNodeErrors('node-1')[0].code).toBe('PORT_NOT_CONNECTED');
  });
  ...
  it('does not call validateGraph when only node position changes', () => {
    ...
    const movedNode: PipelineNode = { ...mockNode, position: { x: 100, y: 200 } };
